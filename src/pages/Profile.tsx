...
export default function Profile() {
  const { user: authUser, uid, loading: authLoading } = useAuth();
    ...
    try {
      ...
      if (auth.currentUser) {
        ...
        if (demoUserStr) {
          ...
          const updatedDemoUser = { ...demoUser, name, department, profilePic };
          localStorage.setItem('hr_pulse_demo_user', JSON.stringify(updatedDemoUser));
        }
      }
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (!auth.currentUser) {
      toast.info('Password change is disabled in Demo Mode');
      return;
    }
